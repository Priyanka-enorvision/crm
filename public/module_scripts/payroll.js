$(document).ready(function () {
	...
	function initializePayrollTable(staffId = 0) {
		return $('#xin_table').DataTable({
			"bDestroy": true,
			"ajax": {
				url: main_url + "payroll/payslip_list/" + staffId + "/" + $('#month_year').val(),
				type: 'GET'
			},
			"language": {
				"lengthMenu": dt_lengthMenu,
	...
	var xin_table = initializePayrollTable();
	...
	$("#set_salary_details").submit(function (e) {
		...
		var xin_table2 = initializePayrollTable(staff_id);

		xin_table2.ajax.reload(function (json) {
			$('#get_date').html($('#month_year').val());
		}, false) // false parameter means "keep paging"
			.always(function () {
				laddaButton.stop();
			});
	});
