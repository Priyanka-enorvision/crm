...
<script>
  ...
  //         var subtotal = qty * price;
  //         row.find('.sub_total_item').val(subtotal.toFixed(2));
  //         calculateTotals(); // Recalculate totals whenever an item's subtotal is updated
  //     }

  //     // Function to calculate overall totals
  //     function calculateTotals() {
  //         let itemsSubtotal = 0;
  //         $('.sub_total_item').each(function() {
  //             itemsSubtotal += parseFloat($(this).val()) || 0;
  //         });
  //         $('.items-sub-total').val(itemsSubtotal.toFixed(2));

  //         const discountType = $('.discount_type').val();
  //         const discountFigure = parseFloat($('.discount_figure').val()) || 0;
  //         let discountAmount = 0;

  //         if (discountType == '1') { // Flat
  //             discountAmount = discountFigure;
  //         } else if (discountType == '2') { // Percentage
  //             discountAmount = (itemsSubtotal * discountFigure) / 100;
  //         }

  //         $('.discount_amount').val(discountAmount.toFixed(2));

  //         let subtotalAfterDiscount = itemsSubtotal - discountAmount;
  //         $('.sub_total').text(subtotalAfterDiscount.toFixed(2));
  ...
  //         $('.fgrand_total').val(grandTotal.toFixed(2));
  //     }

  //     // Function to setup event listeners
  //     function setupEventListeners() {
  //         // Attach event listeners to input fields for recalculations
  //         $('#task_list').on('input', '.qty_hrs, .unit_price', function() {
  //             var row = $(this).closest('.item-row');
  //             calculateSubtotal(row);
  //         });

  //         // Attach event listeners to discount and tax fields for recalculations
  //         $('.discount_type').on('change', calculateTotals);
  //         $('.discount_figure').on('input', calculateTotals);
  //         $('.tax_type').on('change', function() {
  //             const selectedTax = this.options[this.selectedIndex];
  //             const taxRate = selectedTax.getAttribute('tax-rate');
  ...
  //     // Add new item row
  //     $('#task_list').on('click', '#add-invoice-item', function() {
  //         $('#task_list').append(
  //             '<div class="row item-row">' +
  //                 '<div class="form-group mb-1 col-sm-12 col-md-5">' +
  //                     '<label for="item_name">Item</label>' +
  ...
  //             '</div>'
  //         );
  //         setupEventListeners(); // Reapply event listeners to newly added rows
  //     });

  //     // Handle project selection change
  //     $('#project_select').change(function() {
  //         var project_id = $(this).val();
  //         $('#task_list').empty(); // Clear existing tasks
